# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 19:28:37 2019

@author: Deepika


#Instructions to run the code:
#1.Place the csv  file provided in the same directory as the python file and run the python using python3.
#2. you can see both results of mncc ad mlcc alogithm as a vizualized graph.In case of mlcc the algorithm prints only the largest connected component.
"""
...
def create_spanning_tree(sensor_nodes, Comm_Range, Budget):
    ...
    while edges_heap:
        ...
        if to not in visited:
            ...
            next_node_list = [((next_node_dict[i][1]['weight']), next_node_dict[i][0]) for i in
                              range(len(next_node_dict))]
...
def BCRP_MNCC(mst, sensor_nodes, Comm_Range, Budget):
    ...
    Rem_nodes = 0
    Conn_Compo = 1
    print("Entering Algo 4: BCRP-MNCC")
    ...
    if (G_mst.size(weight='weight') <= Budget):
        ...
        while (G_mst.size(weight='weight') > Budget):
            ...
            plt.axis('on')
            plt.title(
                "MST graph of Sensor nodes after removal of {nodes} maximum weighted edges ".format(nodes=Rem_nodes))
            plt.show()
        print("Number of connected components:", Conn_Compo)
...
def BCRP_MLCC(Comm_Range, Budget):
    print("Entering algo 5: BCRP_MLCC")
    sensor_nodes = pd.read_csv("sensor_location_2.csv", header=None)
    ...
    n = len(sensor_nodes)
    print("Largest connected Component before running BCRP MLCC:")
    visualise_my_mst(mst)
    for i in range(n, 1, -1):
        if weights_sum <= Budget:
            print("Largest connected Component after reaching budget:")
            visualise_my_mst(mst)
            return mst
        else:
...
if __name__ == '__main__':
    ...
    # BCRP_MLCC is reading the csv file in its function definition
    # Reading from csv file for BCRP_MNCC
    sensor_nodes = pd.read_csv("sensor_location_2.csv", header=None)
